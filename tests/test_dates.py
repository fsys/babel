...
class DateTimeFormatTestCase(unittest.TestCase):
    ...
    def test_month_context(self):
        ...
        self.assertEqual('2', fmt['MMMMM']) # narrow format
        fmt = dates.DateTimeFormat(d, locale='cs_CZ')
        self.assertEqual('ú', fmt['LLLLL']) # narrow standalone
...
class FormatDatetimeTestCase(unittest.TestCase):
    ...
    def test_with_float(self):
        ...
        self.assertEqual('April 1, 2012 at 3:30:29 PM +0000', formatted_string)
...
class FormatTimeTestCase(unittest.TestCase):
    ...
    def test_with_float(self):
        ...
        self.assertEqual('3:30:29 PM +0000', formatted_time)
...
def test_get_timezone_name():
    ...
    assert dates.get_timezone_name(dt, width='short', locale='en_US') == 'PST'

    tz = timezone('America/Los_Angeles')
    assert dates.get_timezone_name(tz, locale='en_US') == 'Pacific Time'
    assert dates.get_timezone_name(tz, 'short', locale='en_US') == 'PT'
    ...
    tz = timezone('America/St_Johns')
    assert dates.get_timezone_name(tz, locale='de_DE') == 'Neufundland-Zeit'

    tz = timezone('America/Los_Angeles')
    assert dates.get_timezone_name(tz, locale='en', width='short',
                                   zone_variant='generic') == 'PT'
    assert dates.get_timezone_name(tz, locale='en', width='short',
                                   zone_variant='standard') == 'PST'
    assert dates.get_timezone_name(tz, locale='en', width='short',
                                   zone_variant='daylight') == 'PDT'
    assert dates.get_timezone_name(tz, locale='en', width='long',
                                   zone_variant='generic') == 'Pacific Time'
    assert dates.get_timezone_name(tz, locale='en', width='long',
                                   zone_variant='standard') == 'Pacific Standard Time'
    assert dates.get_timezone_name(tz, locale='en', width='long',
                                   zone_variant='daylight') == 'Pacific Daylight Time'
...
def test_format_datetime():
    ...
    full = dates.format_datetime(dt, 'full', tzinfo=timezone('Europe/Paris'),
                                 locale='fr_FR')
    assert full == ('dimanche 1 avril 2007 17:30:00 heure '
...
def test_format_time():
    ...
    assert fr == '15:30:00 heure avanc\xe9e d\u2019Europe centrale'
    ...
    assert paris == '15:30:00 heure normale de l\u2019Europe centrale'
